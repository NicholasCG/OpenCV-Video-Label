...
prelu_initializer = tf.compat.v1.constant_initializer(0.25)

def alexnet_conv_layers(input, batch_size, num_unrolls):
    input = tf.cast(input, dtype=tf.float32) - IMAGENET_MEAN
    with tf.compat.v1.variable_scope('conv1'):
        conv1 = tf_util.conv_layer(input, 96, 11, 4, padding='VALID')
        pool1 = tf.nn.max_pool2d(
                input=conv1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID',
                name='pool1')
    ...
    with tf.compat.v1.variable_scope('conv1_skip'):
        ...
        conv1_skip = tf_util.prelu(tf_util.conv_layer(lrn1, 16, 1, activation=None),
                prelu_skip)
        conv1_skip = tf.transpose(a=conv1_skip, perm=[0,3,1,2])
    ...
    with tf.compat.v1.variable_scope('conv2'):
        ...
        pool2 = tf.nn.max_pool2d(
                input=conv2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID',
                name='pool2')
    ...
    with tf.compat.v1.variable_scope('conv2_skip'):
        ...
        conv2_skip = tf_util.prelu(tf_util.conv_layer(lrn2, 32, 1, activation=None),
                prelu_skip)
        conv2_skip = tf.transpose(a=conv2_skip, perm=[0,3,1,2])
    ...
    with tf.compat.v1.variable_scope('conv5'):
        ...
        pool5 = tf.nn.max_pool2d(
                input=conv5, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID',
                name='pool5')
        pool5 = tf.transpose(a=pool5, perm=[0,3,1,2])
    ...
    with tf.compat.v1.variable_scope('conv5_skip'):
        ...
        conv5_skip = tf_util.prelu(tf_util.conv_layer(conv5, 64, 1, activation=None),
                prelu_skip)
        conv5_skip = tf.transpose(a=conv5_skip, perm=[0,3,1,2])
    ...
    with tf.compat.v1.variable_scope('re3', reuse=reuse):
        ...
        with tf.compat.v1.variable_scope('lstm1'):
            ...
            if prevLstmState is not None:
                state1 = tf.nn.rnn_cell.LSTMStateTuple(prevLstmState[0], prevLstmState[1])
            else:
        ...
        with tf.compat.v1.variable_scope('lstm2'):
            ...
            if prevLstmState is not None:
                state2 = tf.nn.rnn_cell.LSTMStateTuple(prevLstmState[2], prevLstmState[3])
            else:
...
def loss(outputs, labels):
    with tf.compat.v1.variable_scope('loss'):
        diff = tf.reduce_sum(input_tensor=tf.abs(outputs - labels, name='diff'), axis=1)
        loss = tf.reduce_mean(input_tensor=diff, name='loss')
